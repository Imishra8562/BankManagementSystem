    ...
    String btnval = request.getParameter("b1");
        try {
            ...
            if (btnval.equalsIgnoreCase("Submit")) {
                ...
                ps = con.prepareStatement("INSERT INTO Customers(username,name,gender,contact,address,email,aadhar_number,pancard_number)VALUES (?, ?,?, ?, ?, ?,?,?)");
                ps.setString(1, t1);
                ps.setString(2, t2);
             ...
             // Retrieve customer_id
                ...
                // Search operation
                String t1 = request.getParameter("t1");
                ps = con.prepareStatement("SELECT  C.customer_id,C.username,A.account_id,C.name,C.gender,C.contact,C.address,C.email,C.aadhar_number,C.pancard_number FROM Customers C JOIN  Accounts A ON C.customer_id = A.customer_id WHERE C.customer_id =?");
                ps.setString(1, t1);
                ResultSet rs = ps.executeQuery();
                if (rs.next()) {
            %>
            ...
            <%
                ...
                String t4 = request.getParameter("t4");
                String t5 = request.getParameter("t5");
                ps = con.prepareStatement("UPDATE Customers SET name=?, contact=?, address=?, email=?, WHERE customer_id=?");
                ps.setString(1, t2);
                ps.setString(2, t3);
                ps.setString(3, t4);
                ps.setString(4, t5);
...
%>
	 ...
	 <!--  101102   "SELECT * FROM Customers WHERE Customer_id=?" -->    